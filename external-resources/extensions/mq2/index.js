const tubitv2mq2 = formatMessage => ({
    name: formatMessage({
        id: 'tubitv2mq2.name',
        default: '氣體感測器'
    }),
    extensionId: 'tubitv2mq2',
    version: '1.0.0',
    supportDevice: ['arduinoUno', 'arduinoNano', 'arduinoLeonardo', 'arduinoMega2560', 'arduinoEsp32', 'arduinoEsp8266'],
    author: 'HUNG TU 鴻兔科技',
    iconURL: `assets/tubitv2mq2.png`,
    description: formatMessage({
        id: 'tubitv2mq2.description',
        default: '感測液化氣、丁烷、丙烷、甲烷、煙霧等的探測'
    }),
    featured: true,
    blocks: 'blocks.js',
    generator: 'generator.js',
    toolbox: 'toolbox.js',
    translations: 'translations.js',
    official: true,
    tags: ['sensor'],
    helpLink: 'https://drive.google.com/drive/folders/1eDjsCnKLM8Wl6UB95v1c3TjAhB84cwa4'
});

module.exports = tubitv2mq2;
